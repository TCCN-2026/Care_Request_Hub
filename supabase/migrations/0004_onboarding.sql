-- Onboarding: the only way to create an organisation. Runs as a single
-- SECURITY DEFINER transaction so a new provider/supplier org, its owner
-- membership and the user's profile are always created together - there is
-- deliberately no INSERT policy on `organisations` for regular users.
...
begin
  ...
  values (
    ...
    case when p_org_type = 'supplier' then 'pending_verification' else 'active' end
  )
